...
is known only at runtime. For instance, to stick with the `plot`
...
as arguments to `plot`. Suppose this list is in the variable
`plot-data`. You could invoke `plot` on the values in that
list like this: 
...
That's where `APPLY` comes in. Like `FUNCALL`, the first
argument to `APPLY` is a function object. But after the function
...
takes `&optional`, `&rest`, or `&key` arguments--the
